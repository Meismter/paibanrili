# ShiftScheduler · iOS 排班助手

按月计划排班的本地 iOS 应用（**v1.3**），制作：**Jason Gao**。支持导入 Word/Excel/TXT/微信文本智能识别排班、手动点击排班、快速轮转排班、自定义班次（名称/时间段/颜色）、系统日历双向同步、桌面小组件（大/中/小三种尺寸）、艾森豪威尔矩阵日期备注、深色模式。

## ✨ 最新特性（v1.3）

- **月历滑动翻页**：横向滑动即可切换月份，原生物理滚动完全跟手、无闪烁；箭头按钮与长按跳转保留
- **长按年月快速跳转**：长按标题弹出「左年右月」双滚轮选择器，±12 年范围任意跳转
- **艾森豪威尔矩阵日期备注**：点选/长按日期可编辑四象限备注（重要且紧急 / 重要不紧急 / 紧急不重要 / 不重要不紧急）；有备注的日期右上角显示象限彩色小圆点（≤4 个）
- **多月份日历同步**：同步/导入前可按年份分组勾选月份（前后各 12 个月共 25 个月），一键全选
- **性能优化**：排班条目按日预分桶、班次索引缓存、数据库谓词下推，翻页与渲染零卡顿
- **深色模式**：跟随系统 / 浅色 / 深色三档
- 小组件：Small=当前/下一班次、Medium=未来 7 天、Large=未来 30 天
...
5. 小组件验证：模拟器桌面长按 → 添加「排班助手」小组件，可切换大/中/小三种尺寸
...
| 日期备注（艾森豪威尔矩阵） | 点选日期 →「日期备注」，或长按日期格 |
| 快速切换月份 | 长按年月标题（滚轮）或横向滑动翻页 |
| 返回今日 | 月历右下角「今日」悬浮按钮（非本月时显示） |
| 班次自定义（名称/时间/颜色） | 班次 Tab |
| 成员管理（增删改） | 设置 Tab → 成员管理 |
| 同步到日历 / 从日历导入（可勾选月份） | 设置 Tab → 日历同步 |
| 深色模式 | 设置 Tab → 外观 |
...
- 备注独立存储（`DayNote` 模型，按日唯一），与排班班次互不影响
...
2. 仓库 **Actions** 页面 → 左侧选中 **Build IPA** → 点 **Run workflow** 手动触发（或推送 main 自动触发）
...
5. 构建成功后 IPA 会自动发布到 **GitHub Packages**（`@meismter/shift-scheduler-ipa`），可在仓库 Packages 页面查看下载

⚠️ 免费签名限制：① 不支持 App Group → **小组件无法读取排班数据（显示空白）**，主 App 功能不受影响；② 签名有效期 7 天，到期需重新签名安装；③ 同时最多 3 个免费签名应用。

## 赞助支持

如果这个应用对你有帮助，欢迎赞助一杯咖啡支持继续开发：

**TRC20：`TNajMHZePUgPa36mxQL8kBXMAgjX9qwZ5f`**
